...
library. It makes no network request other than to the local Ollama. It moved
to `scripts/probe0/` in step 1, unchanged; the reports it produced live in
`scripts/probe0/results/`, and step 5 calibrates the estimator against them.
...
CGO_ENABLED=0 GOOS=darwin  GOARCH=arm64 go build -trimpath -o dist/probe0-darwin-arm64      ./scripts/probe0
CGO_ENABLED=0 GOOS=linux   GOARCH=amd64 go build -trimpath -o dist/probe0-linux-amd64       ./scripts/probe0
CGO_ENABLED=0 GOOS=windows GOARCH=amd64 go build -trimpath -o dist/probe0-windows-amd64.exe ./scripts/probe0
```
