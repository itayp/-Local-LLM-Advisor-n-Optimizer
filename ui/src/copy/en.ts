...
export const en = {
  ...
  screens: {
    ...
    watch: {
      title: 'New models',
      lead: 'A daily check for new models in the curated families, and new repositories from their makers — quietly, in the background. It never downloads or switches a model by itself.',
      checkNow: 'Check now',
      checking: 'Checking…',
      checkFailed: (message: string) => `The check did not finish: ${message}`,
      running: 'A check is already running; its result will appear below when it finishes.',
      ...
      outcome: {
        ...
        flagged_for_curator: 'New from a maker',
        error: 'Error',
      },
      trigger: {
        scheduler: 'automatic',
      ...
      refreshError: (message: string) => `The catalogue could not be refreshed first: ${message}`,
      maintainerError: (message: string) => `Some makers could not be checked: ${message}`,
    },
    settings: {
      title: 'Settings',
      ...
      notificationsHelp: 'The daily check for new models (New models, in the navigation) uses the purposes picked on Recommend.',
      watchEnabledLabel: 'Check for new models',
      watchEnabledHelp: 'Off pauses the daily check entirely: no refresh, no comparison, nothing logged.',
      watchModeLegend: 'Desktop notification',
      watchMode: {
        on: 'On — notify me when a new model fits and beats what I have',
        quiet: 'Quiet — keep the log, no popup',
        never: 'Never — do not evaluate for notification at all',
      },
      updatesTitle: 'Updates',
      updatesCheck: 'Check for updates',
      updatesChecking: 'Checking…',
      updatesUpToDate: (v: string) => `You're on the latest version (${v}).`,
      updatesAvailable: (v: string) => `Version ${v} is available.`,
      updatesDevBuild: (v: string) =>
        `The newest published release is ${v}. This build was made from source, not downloaded, so there's nothing to compare it to directly.`,
      updatesDownload: 'Download',
      updatesFailed: (message: string) => `Could not check for updates: ${message}`,
      version: (v: string) => `version ${v}`,
